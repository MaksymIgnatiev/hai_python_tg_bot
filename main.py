...
def update_message(obj: User, chatId: int): bot.edit_message_text(format_message(obj), chatId, obj.msg)


@bot.message_handler(commands=["start"])
def start_command(msg: types.Message): bot.send_message(msg.chat.id, "Привіт\\. Я телеграм бот для гри в [шибеницю](https://uk.m.wikipedia.org/wiki/%D0%A8%D0%B8%D0%B1%D0%B5%D0%BD%D0%B8%D1%86%D1%8F_\\(%D0%B3%D1%80%D0%B0\\)) написаний на [Python](https://www.python.org/) бібліотеці [telebot](https://pypi.org/project/telebot/)\\.\n\nДля подробиць використовуйте команду: /help", "MarkdownV2", disable_web_page_preview=True)


@bot.message_handler(commands=["game"])
def game_command(msg: types.Message):
...
def handle_guess(msg: types.Message):
    ...
    if text == "/resend": return user.__setattr__("msg", bot.send_message(chatId, format_message(user)).message_id)
...
def help_command(msg: types.Message): bot.send_message(msg.chat.id, "Допомога з боту:\n\n/start - запуск/перезапуск бота\n/help - вивести це повідомлення\n/game - почати нову гру (якщо ще не була розпочата)\n\nПід час гри також будуть доступні інші команди. Їх також можна дізнатися відіславши команду /help, але вже під час гри.")


@bot.message_handler()
def message(msg: types.Message): bot.send_message(msg.chat.id, f"Це не валідна команда для бота.  Список доступних команд можна дізнатися надіславши команду: /help") 
